...
namespace ImobiliariaMathers.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> CadastrarUsuario([FromBody] CadastroUsuarioRequest request)
        {
            ...
            var usuario = new Usuario
            {
                ...
                Email = email,
                Senha = _passwordHasherService.HashPassword(request.Senha.Trim()),
        ...
        public async Task<IActionResult> ValidarLogin([FromBody] LoginUsuarioRequest request)
        {
            ...
            if (usuario is null || !_passwordHasherService.VerifyPassword(senha, usuario.Senha))
            {
                return Unauthorized(new { mensagem = "E-mail ou senha incorretos." });
            }

            if (_passwordHasherService.IsLegacyHash(usuario.Senha))
            {
                usuario.Senha = _passwordHasherService.HashPassword(senha);
                await _context.SaveChangesAsync();
            }
